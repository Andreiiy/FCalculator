package com.atatar.fructioncalculator;
...
public class Calculator {
    ...
    public String getMonitorString(Boolean result){

        if(result) {
            String resaltMonitorString = "$$\\"+getTextSize(result)+"{"
                    + integerResalt + "\\frac{" + fract1Resalt + "}{" + fract2Resalt + "}}$$";

            return resaltMonitorString;
        }

        monitorString = "$$\\"+getTextSize(result)+"{" + integer1 + "\\frac{" + fract11 + "}{" + fract12 + "}  "
                + znak + "  "
    ...
    private String getTextSize(Boolean result){
        Integer strigSize;
        if(!result){
            strigSize = integer1.length() + integer2.length() + integerResalt.length()
                    + fract1Resalt.length() + fract2Resalt.length()+ znakResalt.length();

            if(fract11.length() > fract12.length())
                strigSize = strigSize + fract11.length();
            else strigSize = strigSize + fract12.length();
            if(fract21.length() > fract22.length())
                strigSize = strigSize + fract21.length();
            else strigSize = strigSize + fract22.length();
        }else {
            strigSize = integerResalt.length()
                    + fract1Resalt.length() + fract2Resalt.length()+ znakResalt.length();
        }
        String hightLetters = "Huge";
        if (strigSize>12 && strigSize<=14)
            hightLetters = "huge";  //20
        else if (strigSize>14 && strigSize<=21)
            hightLetters = "Large";  //26
        else if (strigSize>21 && strigSize<=25)
            hightLetters = "large";  //
        else if (strigSize>25 )
            hightLetters = "small";
